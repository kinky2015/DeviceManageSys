...
<t:datagrid name="storageDetailList" title="入库单明细记录" pagination="true" actionUrl="storageDetail.do?datagrid&storageDocNo=${storageDocNo}" fit="true" fitColumns="true" idField="id" queryMode="group" sortName="price" sortOrder="asc">
	...
	<t:dgToolBar title="保存数据" icon="icon-save" funname="saveDetail"></t:dgToolBar>
	<t:dgToolBar title="新增入库记录" icon="icon-add" funname="addDetail"></t:dgToolBar>
...
<div style="border:0px solid red;width:100%">
	<table style="width:100%">
		...
		<tr>
			...
			<td><lable>入库人员：</lable><input type="text" readonly="readonly" id="userName" value="${sessionScope.LOCAL_CLINET_USER.userName}"></td>
...
<script>
	function saveDetail()
	{
		var dataObj = $("#storageDetailList").datagrid("getData");
		if(dataObj == null || dataObj.total == 0)
		{
			return;
		}
		...
		var postData = {"docInfo":JSON.stringify(docInfo), "details": JSON.stringify(dataObj.rows)}
		
		$.ajax({
			url: 'storageDetail.do?savaDetail',
			dataType: 'json',
			type: 'post',
			data: postData,
	...
	function addDetail()
	{
		...
		$.dialog({
		    ...
		    ok: function()
		    {
		    	...
		    	var data = getFormJonsData($('#formobj', iframe.document));
		    	var storageDocNo = $("#storageDocNo").val();
		    	data['doc.storageDocNo'] = storageDocNo;
		    	$("#storageDetailList").datagrid('insertRow',{index:0, row:data})
